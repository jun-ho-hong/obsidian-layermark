...
describe("AnnotationStorage image lifecycle", () => {
  it("renames sidecar and preview files when the source image is renamed", async () => {
    ...
    expect(files.has("assets/a/renamed.jpg.skitch.json")).toBe(true);
    expect(files.has("assets/a/renamed.jpg.skitch.png")).toBe(true);
    expect(JSON.parse(contents.get("assets/a/renamed.jpg.skitch.json") ?? "{}").imagePath).toBe("assets/a/renamed.jpg");
  });
